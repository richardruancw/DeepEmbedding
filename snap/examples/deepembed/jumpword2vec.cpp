...
namespace JumpWord2Vec {
  ...
  void TrainModel(TVVec<TInt, int64>& WalksVV, int& Dimensions, int& WinSize, int& Iter, bool& Verbose,
     ...
     int64 CurrWalk, TRnd& Rnd, TVVec<TFlt, int64>& SynNeg, TVVec<TFlt, int64>& SynPos, TInt& NewFakeWord)  {
  ...
  void LearnEmbeddings(TVVec<TInt, int64>& WalksVV, int& Dimensions, int& WinSize,
   int& Iter, bool& Verbose, TIntFltVH& EmbeddingsHV, TInt& FakeNodeId) {
    ...
    TInt NewFakeNodeId = RnmH.GetDat(FakeNodeId);
  ...
  #pragma omp parallel for schedule(dynamic)
      for (int64 i = 0; i < WalksVV.GetXDim(); i++) {
        TrainModel(WalksVV, Dimensions, WinSize, Iter, Verbose, KTable, UTable,
         WordCntAll, ExpTable, Alpha, i, Rnd, SynNeg, SynPos, NewFakeNodeId); 
      }
    }
    if (Verbose) { printf("\n"); fflush(stdout); }
    for (int64 i = 0; i < SynPos.GetXDim(); i++) {
