...
def load_train_test(eval_path, embedding_path, ours, d, sd, option, game_round):
	train = np.loadtxt(os.path.join(eval_path,"train"+str(game_round)))
	test = np.loadtxt(os.path.join(eval_path,"test"+str(game_round)))
	batch_size_test = test.shape[0]
	if ours == 1:
		embeddings = np.loadtxt(os.path.join(embedding_path,"outemb"+str(game_round)+".txt"), skiprows = 1)
	else:
...
def down_stream_eval(batch_size, d, game_round, ours, decomp, sd=None):
	...
	accuracy, f1 = train_and_test(eval_path, embedding_path, ours, d, sd, 'n+s', logger, game_round, batch_size)
	if ours == 1:
		if sd is None:
			sd = d
		with open(os.path.join(output_stats_path, "down_stream_results"),"a") as f:
				f.write(" ".join([str(game_round), str(accuracy), str(f1), 'n+s']))
				f.write("\n")
		if decomp == 1:
			accuracy, f1 = train_and_test(eval_path, embedding_path, ours, d, sd, 's', logger, game_round, batch_size)
			with open(os.path.join(output_stats_path, "down_stream_results"),"a") as f:
				f.write(" ".join([str(game_round), str(accuracy), str(f1), 's']))
				f.write("\n")
			accuracy, f1 = train_and_test(eval_path, embedding_path, ours, d, sd, 'n', logger, game_round, batch_size)
			with open(os.path.join(output_stats_path, "down_stream_results"),"a") as f:
				f.write(" ".join([str(game_round), str(accuracy), str(f1), 'n']))
				f.write("\n")

	else:
		with open(os.path.join(output_stats_path, "down_stream_results_origin"),"a") as f:
			f.write(" ".join([str(game_round), str(accuracy), str(f1), 'origin']))
			f.write("\n")
...
if __name__ == "__main__":
	...
	down_stream_eval(batch_size, d, game_round, ours, decomp, sd)
